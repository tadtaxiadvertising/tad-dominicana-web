...
export default function TadLandingApp() {
  ...
  return (
    <div className="min-h-screen bg-[#09090c] text-gray-200 font-sans selection:bg-[#FFD400] selection:text-gray-900 flex flex-col relative overflow-hidden">
      ...
      <a href="https://wa.me/18495043872?text=Hola,%20quiero%20más%20información%20sobre%20cómo%20ser%20conductor%20TAD." target="_blank" rel="noopener noreferrer" className="fixed bottom-24 md:bottom-6 right-6 bg-green-500 text-white p-4 rounded-full shadow-lg hover:bg-green-400 hover:scale-110 transition-all animate-bounce z-50 flex items-center justify-center">
        <Smartphone className="w-6 h-6" />
      </a>
      {currentView === 'landing' && (
        <div className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t border-white/10 bg-[#0c0c10]/96 backdrop-blur-xl p-4">
          <div className="mx-auto max-w-md flex gap-3">
            <button onClick={() => navigateTo('register')} className="flex-1 bg-[#FFD400] text-gray-900 rounded-2xl px-4 py-3 font-black uppercase tracking-[0.18em] text-xs shadow-[0_0_24px_rgba(255,212,0,0.24)]">
              Comenzar
            </button>
            <a href="#faq" className="flex-1 border border-[#2c2c34] bg-[#15151b] text-white rounded-2xl px-4 py-3 font-black uppercase tracking-[0.18em] text-xs text-center">
              Ver dudas
            </a>
          </div>
        </div>
            ...
            Gana dinero extra mientras trabajas. Instalamos una pantalla de 10&quot; en tu vehículo para mostrar anuncios y ayudarte a generar más ingresos.
              ...
              <div className="flex flex-wrap justify-center xl:justify-start gap-3 mb-8">
                {heroHighlights.map((item) => (
                  <div key={item} className="rounded-full border border-[#2f2f37] bg-[#131318] px-4 py-2 text-[11px] font-black uppercase tracking-[0.18em] text-white/85">
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-3xl border border-[#FFD400]/20 bg-[#FFD400]/10 p-6 md:p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className="rounded-full bg-black/25 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-white">Nombre claro</span>
                <span className="rounded-full bg-black/25 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-white">WhatsApp correcto</span>
                <span className="rounded-full bg-black/25 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-white">Placa validada</span>
              </div>
            </div>
